...
<div class="w3-main" style="margin-left:300px">
    ...
    <div style="width: 40%; padding-left: 5%">
        <form id="from02" method="post" action="request.php">
...
<script>
    ...
    function user_location() {
        let testValue2 = <?=json_encode($data)?>;
        let mapOptions = {
    ...
    function call() {
        ...
        locations.forEach(element => {
            new L.Marker([element.lat,element.long]).addTo(map)
                .on("mouseover",event =>{
                    event.target.bindPopup('<div class="card"><h3>'+element.title+'</h3></br><h5>'+element.number+'</h5></div>',popupOption).openPopup();
                })
                .on("mouseout", event => {
                    event.target.closePopup();
                })
