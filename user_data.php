...
if (!$connection){
    ...
    $query = "SELECT * FROM user_info WHERE user_id='{$_user_id}'";
    $result = mysqli_query($connection, $query);
    if (mysqli_num_rows($result)>0){
        $data = mysqli_fetch_assoc($result);
        $_username = $data['user_name'];
        $_userblood = $data['blood_group'];
        ...
        <div class="column column-60 column-offset-20">
            ...
            <div class="formc">
                <form id="from01" method="post" action="action.php">
                    ...
                    <input type="text" id="name" name="name" value="<?php echo($_username); ?>">
                    <label for="email">Email</label>
                    <input type="text" id="email" name="email" value="<?php echo($_useremail); ?>">
                    <label for="blood">Blood Group</label>
                    <input type="text" id="blood" name="blood" value="<?php echo($_userblood); ?>">
                    <label for="number">Phone Number</label>
                    <input type="text" id="number" name="number" value="<?php echo($_usernumber); ?>">
                    <input type="hidden" id="userid" name="userid" value="<?php echo($_SESSION['id']); ?>">

                    <input class="button-primary" type="submit" value="submit">
                    <input type="hidden" name="action" id="action" value="update"><a href="dashboard.php">    | Cancel</a>
                </form>
            </div>
        </div>
    </div>
